...
namespace Rackspace.CloudFiles.Unit.Tests.Domain.Request.GetAuthenticationSpecs
{
    ...
    public class when_authenticating_against_cloud_files_and_no_auth_url_provided
    {
        ...
        {
            Assert.That(getAuthentication.CreateUri().ToString(), Is.EqualTo(CloudFiles.Utils.Constants.US_AUTHURL));
        }
        ...
        public void should_have_api_key_in_the_headers()
        {
            ...
            _mockrequest.SetupGet(x => x.Headers).Returns(webrequest);
            getAuthentication.Apply(_mockrequest.Object);
            Assert.That(webrequest[CloudFiles.Utils.Constants.X_AUTH_KEY], Is.EqualTo("apikey"));
        }
    }
